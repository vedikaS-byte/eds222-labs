...
```{r}
# USe qnorm() for calculating the range the 95% confidence interval 
ci <- qnorm(c(0.025, 0.975), mean = diff_props, sd = se_diff_props)
ci
```

**Q10: How would you interpret this confidence interval**

We are 95% confident that the true difference in proportions (population parameter) between the two treatments is about -.0569 and .3396.

## Additional Questions

**Q11: What are permutation tests and mathematical models [both]{.underline} trying to estimate for hypothesis tests?**

Both are trying to estimate the sampling distribution of the test statistic assuming the null hypothesis is true.

**Q12: What are bootstrapping and mathematical models [both]{.underline} trying to estimate for confidence intervals?**

For confidence intervals, both methods try to estimate the approximate interval that captures the true population parameter about 95% of the time assuming we have sufficient evidence to disprove the null hypothesis (there is a difference).

**Q13: What area under the curve is relevant to hypothesis tests? What R function calculates it for a mathematical model?**

The area under the curve corresponding to the relevance of the hypothesis tests are towards the right of the test statistic. pnorm() is used to calculate the p-value.

**Q14: What area under the curve is relevant to confidence intervals? What R function calculates it for a mathematical model?**

The area under the curve relevant to confidence intervals are the bounds left and right of .025 and .97 for the 95% confidence interval (qnorm()).
